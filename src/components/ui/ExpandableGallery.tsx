...
const ExpandableGallery: React.FC<ExpandableGalleryProps> = ({ images, className = '' }) => {
  ...
  return (
    <div className={className}>
      ...
      <div
        className={`flex gap-2 h-[220px] md:h-[420px] w-full ${
        ...
        {images.map((image, index) => (
          <motion.div
            ...
            className={`relative cursor-pointer overflow-hidden rounded-2xl ${
              isMobile ? 'shrink-0 w-[78%] snap-center' : ''
            }`}
            style={{ flex: isMobile ? '0 0 auto' : 1 }}
            animate={{ flex: isMobile ? '0 0 auto' : getFlexValue(index) }}
          ...
          >
            <img
              src={image}
              alt={`Instituto Longevitare — ambiente ${index + 1}`}
      ...
      <AnimatePresence>
        {selectedIndex !== null && (
          ...
          >
            ...
            {images.length > 1 && (
              <button
                className="absolute left-4 z-10 text-white hover:text-white/70 transition-colors"
                onClick={goToPrev}
            ...
            <motion.div className="relative max-w-5xl max-h-[90vh] w-full" onClick={(e) => e.stopPropagation()}>
              <motion.img
                ...
                alt={`Instituto Longevitare — ambiente ${selectedIndex + 1}`}
            ...
            {images.length > 1 && (
              <button
                className="absolute right-4 z-10 text-white hover:text-white/70 transition-colors"
                onClick={goToNext}
